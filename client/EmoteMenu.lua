...
function AddEmoteMenu(menu)
    ...
    if Config.SharedEmotesEnabled then
        sharemenu =
            ...
            submenu,
            Config.Languages[lang]["shareemotes"],
            Config.Languages[lang]["shareemotesinfo"],
            "",
            Menuthing,
            Menuthing
        )
        shareddancemenu =
            _menuPool:AddSubMenu(sharemenu, Config.Languages[lang]["sharedanceemotes"], "", "", Menuthing, Menuthing)
    ...
    if not Config.SqlKeybinding then
        ...
        favmenu =
            _menuPool:AddSubMenu(
            submenu,
            Config.Languages[lang]["favoriteemotes"],
            Config.Languages[lang]["favoriteinfo"],
            "",
            Menuthing,
            Menuthing
        )
        ...
        keyinfo =
            NativeUI.CreateItem(
            Config.Languages[lang]["keybinds"],
        ...
        if input ~= nil then
            ...
            for k, v in pairs(RP) do
                if not ignoredCategories[k] then
                    for a, b in pairs(v) do
                        if
                            string.find(string.lower(a), string.lower(input)) or
                                (b[3] ~= nil and string.find(string.lower(b[3]), string.lower(input)))
                         then
            ...
            if #results > 0 then
                local searchMenu =
                    _menuPool:AddSubMenu(
                    lastMenu,
                    string.format(Config.Languages[lang]["searchmenudesc"], #results, input),
                    "",
                    true,
                    Menuthing,
                    Menuthing
                ...
                if favEnabled then
                    local rFavorite =
                        NativeUI.CreateItem(Config.Languages[lang]["rfavorite"], Config.Languages[lang]["rfavorite"])
                ...
                if Config.SharedEmotesEnabled then
                    sharedDanceMenu =
                        _menuPool:AddSubMenu(
                        searchMenu,
                        Config.Languages[lang]["sharedanceemotes"],
                        "",
                        true,
                        Menuthing,
                        Menuthing
                    ...
                    if v.table == "Shared" then
                        ...
                        if otheremotename == nil then
                            ...
                            desc =
                                ...
                                v.name ..
                                    "~w~) " ..
                                        Config.Languages[lang]["makenearby"] .. " (~y~" .. otheremotename .. "~w~)"
                        end
                    else
                        desc =
                            "/e (" ..
                            v.name .. ")" .. (favEnabled and "\n" .. Config.Languages[lang]["searchshifttofav"] or "")
                    end

                    if v.data.AnimationOptions and v.data.AnimationOptions.PropTextureVariations then
                        local item =
                            NativeUI.CreateListItem(v.data[3], v.data.AnimationOptions.PropTextureVariations, 1, desc)
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    if data == Config.Languages[lang]["sharedanceemotes"] then
                        return
                    end
...
function AddInfoMenu(menu)
    ...
    infomenu =
        _menuPool:AddSubMenu(
        menu,
        Config.Languages[lang]["infoupdate"],
        "~h~~y~The RPEmotes Team & Collaborators~h~~y~",
        "",
        Menuthing,
        Menuthing
    )
    ...
    infomenu:AddItem(
        NativeUI.CreateItem(
            '<font color="#FF25B1"><b>nox-rpemotes Repository<b></font>',
            '<font color="#FF25B1">https://github.com/Tnoxious/nox-rpemotes</font>'
        )
    )
    infomenu:AddItem(NativeUI.CreateItem("Thanks ~o~TayMcKenzieNZ 🍐~s~", "~o~TayMcKenzieNZ~s~ for previous work"))
    infomenu:AddItem(
        NativeUI.CreateItem("Thanks <b>Kibook 🐩</b>", "<b>Kibook</b> for the addition of Animal Emotes 🐩 submenu.")
    )	
